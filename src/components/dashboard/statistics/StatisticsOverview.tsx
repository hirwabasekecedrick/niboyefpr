...
import { BarChart3, TrendingUp, Users, Building2, Trophy } from 'lucide-react'
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-6 rounded-2xl border border-red-50 shadow-sm">
                    ...
                    <div className="text-2xl font-black text-slate-800">{stats.totalContributionRwf.toLocaleString()} <span className="text-xs font-medium">RWF</span></div>
                </div>

                <div className="bg-white p-6 rounded-2xl border border-red-50 shadow-sm">
                    <div className="flex items-center gap-3 text-slate-500 text-xs font-bold uppercase tracking-wider mb-2">
                        <Trophy className="w-4 h-4 text-primary" /> Sector Status
                    </div>
                    <div className="text-2xl font-black text-slate-800">Excellent</div>
                </div>
            </div>
                        ...
                        {stats.monthlyData.map((d: any) => (
                            <div key={d.month} className="flex-1 flex flex-col items-center gap-2 group">
                                <div
                                    ...
                                    style={{ height: `${(d.amount / Math.max(...stats.monthlyData.map((m: any) => m.amount), 1)) * 100}%` }}
                                >
                        ...
                        {grading.slice(0, 5).map((item) => (
                            <div key={item.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 hover:bg-red-50 transition-colors">
                                ...
                                <div className="text-sm font-black text-primary">
                                    {item.totalContribution.toLocaleString()} RWF
                                </div>
                            </div>
                        ))}
                    </div>
